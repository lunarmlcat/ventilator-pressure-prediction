...
class Net13(nn.Module):
    def __init__(self, net_params):
        ...
        self.input_dim = 4 + len(net_params.cont_seq_cols)
        self.input_len = 80
        self.output_len = 80
        self.hidden_size = net_params.hid_size
        self.num_levels = net_params.num_levels
        self.groups = net_params.groups
        self.modified = net_params.modified
        self.kernel_size = net_params.kernel
        self.dropout = net_params.dropout
        self.single_step_output_One = net_params.single_step_output_One
        self.concat_len = net_params.concat_len
        self.pe = net_params.positionalE
        ...
        self.blocks1 = EncoderTree(
            ...
            INN =  net_params.modified)

        if net_params.num_stacks == 2: # we only implement two stacks at most.
        ...
        self.stacks = net_params.num_stacks
    ...
    def forward(self, cate_seq_x, cont_seq_x):
        ...
        x = torch.cat((r_emb, c_emb, cont_seq_x), 2)

        assert self.input_len % (np.power(2, self.num_levels)) == 0 # evenly divided the input length into two parts. (e.g., 32 -> 16 -> 8 -> 4 for 3 levels) 300 -> 150 -> 
        if self.pe:
