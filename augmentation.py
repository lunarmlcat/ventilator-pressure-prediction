...
class Jitter(AudioTransform):
    def __init__(self, always_apply=False, p=0.5, sigma=0.03, **kwargs):
...
class Permutation(AudioTransform):
    def __init__(self, always_apply=False, p=0.5, max_segments=5, seg_mode="equal", **kwargs):
        super().__init__(always_apply, p)
...
class Magnitude_warp(AudioTransform):
    def __init__(self, always_apply=False, p=0.5, sigma=0.2, knot=4, **kwargs):
        super().__init__(always_apply, p)
        self.sigma = sigma
        self.knot = knot
    ...
    def apply(self, x: np.ndarray, **params):
        ...
        random_warps = np.random.normal(loc=1.0, scale=self.sigma, size=(x.shape[0], self.knot+2, x.shape[2]))
        warp_steps = (np.ones((x.shape[2],1))*(np.linspace(0, x.shape[1]-1., num=self.knot+2))).T
        ret = np.zeros_like(x)
        for i, pat in enumerate(x):
...
class Time_warp(AudioTransform):
    def __init__(self, always_apply=False, p=0.5, sigma=0.2, knot=4, **kwargs):
        super().__init__(always_apply, p)
        self.sigma = sigma
        self.knot = knot
...
class Window_slice(AudioTransform):
    def __init__(self, always_apply=False, p=0.5, reduce_ratio=0.9, **kwargs):
        super().__init__(always_apply, p)
...
class Window_warp(AudioTransform):
    def __init__(self, always_apply=False, p=0.5, window_ratio=0.1, scales=[0.5, 2.], **kwargs):
        super().__init__(always_apply, p)
